...
function createServerCache(store = new MemoryCacheStore()) {
  assert(
  ...
  // guarantee we clear out all cache promises so we dont have a memory leak if
  // forgot to call replaceWitCachedValues after a render
  process.nextTick(() => {
    disableCache();
  });

  return function replaceWithCachedValues(html) {
    const _cachePromises = cachePromises;
    cachePromises = [];

    return Promise.all(_cachePromises).then((cachedValues) => {
      const cacheMap = {};
      let regex = '';
...
ReactCompositeComponent.mountComponent = function mountComponentFromCache(
  ...
  const hashedKey = farmhash.hash64(currentElement.type.cacheKeyFn(currentProps));
...
module.exports.createServerCache = createServerCache;
